...
适用于 **Vue 2**。通过 **插槽** 自定义节点与列表区域（插槽优先级高于 **`render*`** 命令式渲染），使用 **`v-on` / `@`** 监听事件，通过 **`ref`** 调用组件暴露的方法。

> **说明**：Vue 2 封装未使用 **`emits`** 选项声明事件，但模板中仍可使用 **`@expand`、`@select`** 等与下表名称对应；与 `expandedKeys` / `selectedKeys` / `checkedKeys` 的双向绑定请使用 **`.sync` 修饰符**（见下文），而非 Vue 3 的 **`v-model:*`**。
...
| expandedKeys         | 展开的节点 key 集合（受控）；配合 **`.sync`** 见下文                 | `TreeNodeKey[]`                                                      | `undefined` | -                            |
| expandOnClickNode      | 点击节点是否展开                                                     | `Boolean`                                                            | `false`   | -                            |
| selectable           | 是否可选中                                                           | `Boolean`                                                            | `false`   | -                            |
| selectMultiple       | 是否可多选                                                           | `Boolean`                                                            | `false`   | -                            |
| selectedKeys         | 选中的节点 key 集合（受控）；配合 **`.sync`** 见下文                | `TreeNodeKey[]`                                                      | `undefined` | -                            |
| checkable            | 是否显示复选框                                                       | `Boolean`                                                            | `false`   | -                            |
| checkedKeys          | 勾选的节点 key 集合（需在 **`checkable`** 场景下使用）；配合 **`.sync`** 见下文 | `TreeNodeKey[]` | `undefined` | -                            |
...
插槽优先级高于同名 **`render*`** 属性。模板中可使用 **camelCase** 或 **kebab-case**（例如 **`sticky-header`** 对应 **`stickyHeader`**）。
...
## `.sync` 双向绑定（替代 Vue 3 的 `v-model:`）

| 写法示例                           | 对应 prop           | 对应更新事件              |
| ---------------------------------- | ------------------- | ------------------------- |
| **`:expandedKeys.sync`**           | `expandedKeys`      | **`update:expandedKeys`** |
| **`:selectedKeys.sync`**           | `selectedKeys`      | **`update:selectedKeys`** |
| **`:checkedKeys.sync`**            | `checkedKeys`       | **`update:checkedKeys`**  |
