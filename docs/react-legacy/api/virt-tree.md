...
适用于 **React Legacy**（与 React 18 封装 **API 一致**）。树形组件基于 Vanilla 封装，支持 **`ref`**（**`ForwardRef`**）调用实例方法与命令式 **`render*`**；若同时使用 **JSX 形态的 Render Props**（见下文），则 **JSX Render Props 优先级高于对应的 `render*`**。
