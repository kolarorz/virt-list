...
适用于 **react-legacy**。VirtGrid 基于 VirtList 封装：**与 VirtList 相同的 props / 事件保持一致即可，不重复列出**。
...
- 同 VirtList 事件即可（`onScroll` / `onToTop` / `onToBottom` / `onItemResize` / `onUpdate`）
- `onUpdate` 签名：`(renderList: unknown[], state: ListState) => void`
