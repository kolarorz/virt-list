...
<PlaygroundHost framework="react-legacy" example-id="list-chat-collapse" />
...
虚拟列表的项 DOM 由列表自己挂载，**不在 React 的渲染树上**。外层 `setState` 并不会让已经渲染出来的项重新渲染，所以折叠状态放在外层再靠 state 驱动是行不通的。

这个示例把状态放在项内部的 `ChatBubble` 组件里（`useState`）：点击时它自身重渲染，高度变化被 ResizeObserver 捕捉，列表自动修正位置。同时把状态同步到外层的一个 `Set`（用 `useRef` 持有），项滚出渲染窗口被销毁、之后再滚回来时据此恢复。

批量操作（全部展开 / 折叠）改的是外层记录，已渲染的项不会自己更新，需要调用 `forceUpdate()` 让列表重建渲染窗口内的项。

代码与 React 18 版本完全一致，只是从 `@virt-list/react-legacy` 引入（内部用 `ReactDOM.render` 而非 `createRoot`）。
