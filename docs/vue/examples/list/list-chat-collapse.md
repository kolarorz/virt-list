...
<PlaygroundHost framework="vue" example-id="list-chat-collapse" />
...
虚拟列表的项 DOM 由列表自己挂载，**不在外层组件的响应式渲染树上**。外层改一个 `ref` 并不会让已经渲染出来的项重新渲染，所以折叠状态放在外层再靠响应式驱动是行不通的。

这个示例把状态放在项内部的子组件里：点击时子组件自身重渲染，高度变化被 ResizeObserver 捕捉，列表自动修正位置。同时把状态同步到外层的一个 `Set`，项滚出渲染窗口被销毁、之后再滚回来时据此恢复。
