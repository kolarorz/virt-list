...
<PlaygroundHost framework="react" example-id="list-chat-collapse" />
...
批量操作（全部展开 / 折叠）是另一条路径：它改的是外层记录，已渲染的项不会自己更新，需要调用 `forceUpdate()` 让列表重建渲染窗口内的项。

::: tip 事件回调里读最新的 list
`onToggle` / `setAll` 都是长期存活的回调，直接闭包捕获 `list` 会拿到旧值。示例用一个 `listRef` 兜住，每次渲染同步一次。
:::
