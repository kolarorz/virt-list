...
| onLoadStateChange | 加载状态变化（loading / hasMore / pendingNew） | `(loadState: LoadState) => void`      |
...
| renderHeader        | 顶部区域：`(loadState: LoadState) => ReactNode`                       |
| renderFooter        | 底部区域：`(loadState: LoadState) => ReactNode`                       |
...
```js
// 平滑滚动到第 3000 项
listRef.current.scrollToIndex(3000, { behavior: 'smooth' });
// 自定义时长并在结束后做点什么
listRef.current.scrollToTop({ behavior: 'smooth', duration: 600, onDone: (canceled) => {} });
```
...
`getState()` 或 **`onUpdate`** 中 `state` 的类型：
...
| renderBegin    | `number` | 实际渲染结束下标(包含buffer)(包含buffer)                           |
...
async function onLoadMore(direction: LoadDirection) {
  ...
  setList((prev) => (direction === 'top' ? data.concat(prev) : prev.concat(data)));
...
header / footer 拿到的加载状态、`onLoadStateChange` 的回调参数、以及 `getLoadState()`
的返回值都是这个类型：
